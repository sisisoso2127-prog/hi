...
\newtheorem*{theorem}{Th\'eor\`eme 5$'$}
...
\subsection{Théorème 3 --- Dinkelbach à paramètre rationnel}
...
\[
  ...
  \qquad\text{et si } U=0 \text{ alors } q^\star=q .
\]
\end{theorem}
...
\noindent
La version d'origine (Th. 5) prend $\Rcal=\Scal$ et
$D_{\min}=\min_{x\in\Scal}D(x)$ ; comme le minimum de $D^{+}$ porte sur un
sous-ensemble, $D^{+}\ge D_{\min}$ et la borne est mécaniquement plus fine.
