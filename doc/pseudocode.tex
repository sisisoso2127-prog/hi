...
multi-objectif en nombres entiers\\[6pt]
\normalsize\itshape Pseudo-code des algorithmes}
\author{}
\date{}
...
\begin{algorithm}[H]
\caption{\textsc{TestEfficacité}$(\bar x,\tau)$ --- Théorème 2 (test d'efficacité)}
...
\begin{algorithm}[H]
\caption{\textsc{Dinkelbach}$(f,X)$ --- Théorème 3 (convergence finie)}
...
\begin{algorithm}[H]
\caption{\textsc{AjouterCoupe}$(\Rcal,\bar x)$ --- Théorème 4 (coupe de dominance)}
...
$U\ge\max\{0,\ F_{\Rcal}(q)\}$ avec
$F_{\Rcal}(q)=\max_{x\in\Rcal}\{QN(x)-PD(x)\}$, et
$D^{+}=\min\{D(x) : x\in\Rcal,\ QN(x)-PD(x)\ge 0\}$, valant $+\infty$ --
donc $U/(QD^{+})=0$ -- si cet ensemble est vide. La troncature à $0$ n'est
pas cosmétique : dès qu'une coupe a retranché tout ce qui dépassait le
seuil, $F_{\Rcal}(q)$ devient strictement négatif, et un $U$ négatif ferait
descendre la borne sous $q$. Alors
\[
  q^\star\ \le\ q+\frac{U}{Q\,D^{+}},
  \qquad\text{et si } F_{\Rcal}(q)\le 0 \text{ alors } q^\star=q .
\]
\end{theorem}
...
borne à un seuil \emph{libre}. Nous avons longtemps écrit que les deux
énoncés étaient incomparables et qu'il fallait en prendre le minimum ;
c'est faux, et les deux démonstrations se composent. Pour tout seuil
rationnel $t$, tout $v\ge\max_{\Rcal}(N-tD)$ avec $v\ge 0$, et
$D^{+}_{t}=\min\{D(x):x\in\Rcal,\ N(x)-tD(x)\ge 0\}$ ($=+\infty$ si vide),
on a $\max_{\Rcal}f\le t+v/D^{+}_{t}$. Le théorème~5 en est le cas $t=q$ ;
le théorème~8 celui où $D^{+}_{t}$ est remplacé par le $D_{\min}$ plus
faible. La forme unifiée \emph{domine les deux}, et non leur minimum, au
prix d'un seul programme supplémentaire au seuil retenu.

\begin{algorithm}[H]
\caption{\textsc{Certifier}$(q,\mathcal{D},B,\lambda,T)$ --- Théorème 5 (borne resserrée)}
...
$D_{\min}\gets\bot$\;
$\mathcal{P}\gets$ $\mathcal{D}$ dédoublonné, dans son \textbf{ordre
d'insertion}
  \Comment*[r]{stable par préfixe, donc borne monotone}
\For{$t\gets 1$ \KwTo $T$}{
