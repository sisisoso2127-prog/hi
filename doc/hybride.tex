...
\begin{remark}[ce que cette hétérogénéité coûte, et à qui]
Elle joue \emph{contre} la méthode, non en sa faveur. Les tableaux de
comparaison avec la littérature (§\ref{sec:matchzm}, §\ref{sec:matchdrici})
sont en \conf{0}{0}{0} : ils opposent aux méthodes publiées la plus faible
des trois versions de la nôtre. De même la section~\ref{sec:cglp} raisonne
sur un écart de $98{,}6\ \%$ là où la configuration courante en donne
$77{,}2$ sur la même instance -- ce qui ne change pas sa conclusion
(\emph{lever le plafond ne suffit pas}) mais en change les chiffres. Les
remesurer est le premier des travaux restants ; nous préférons le marquer
que le taire.
\end{remark}
...
\begin{table}[htbp]
\centering
\caption{Match, $24$ instances, vérité terrain par énumération.\ \conf{0}{0}{0}}
\label{tab:match}
\begin{tabular}{lcc}
\toprule
