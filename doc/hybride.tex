...
La correction s'est faite en deux temps, et le premier était faux. Nous
avions voulu prendre la décision \emph{au moment où l'information existe} :
après le premier tour, la première route a produit une borne, donc l'écart
est connu ; on réserve alors -- ou non. Cela ne peut pas fonctionner ici,
parce que le premier tour consomme à lui seul la totalité du plafond :
l'oracle interne tourne jusqu'à épuisement, et à la fin du tour~$1$ il ne
reste rien à réserver. Le symptôme était sans équivoque -- statut
« interrompue », zéro tour, zéro appel, gain $+0{,}00$ là où la réserve en
amont gagnait $9{,}82$ points. Le déclencheur ne choisissait pas mal, il ne
s'exécutait pas.

La réserve est donc prise \emph{avant} la boucle, et le déclencheur change
de sens : il ne réserve plus, il \emph{libère}. Si le premier tour montre
une borne déjà serrée, la part réservée est rendue et le tour suivant en
profite ; sinon elle reste acquise. La décision se prend toujours sur une
mesure, avec le seuil qui sert déjà ailleurs à déclarer une borne qui ne
ferme pas -- c'est le sens de la réservation qui s'inverse, pas le moment de
la décision. Une absence de borne au tour~$1$ compte comme une borne qui ne
ferme pas : la réserve est conservée.
...
\begin{remark}[une réserve doit être bornée par ce qu'elle finance]
Une première version réservait un \emph{pourcentage} du budget restant --
$25\ \%$, soit $28$ appels -- à une procédure qui en dépense $2$. Vingt-six
appels étaient donc retirés aux coupes sans contrepartie, et deux lignes
reculaient. Bornée par le nombre de tours que la route peut effectivement
faire, la réserve ne coûte plus que $3$ à $4$ coupes, et la ligne
$n=20$, $\text{corr}=0$, graine~$2$ passe de $-0{,}66$ à $+0{,}33$. Le
paramètre n'a pas été réglé : il a été \emph{dérivé} de la procédure qu'il
finance.
\end{remark}
