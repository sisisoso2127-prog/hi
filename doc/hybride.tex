...
Deux explications de l'écart résiduel ont été avancées, et les deux ont été
réfutées par la mesure : le plafond de coupes
(§\ref{sec:cglp}) et leur ordre. Une troisième restait : la région que l'on
sait retrancher serait négligeable devant ce qui sépare $\Rcal$ de $\Ecal$.
...
domine donc déjà la quasi-totalité de $\Scal^{+}$ : la troisième explication
tombe comme les deux précédentes. Et le résultat a une portée propre, en
...
$\max_{x\in\Rcal}G_t(x)\le v$ avec $v\ge 0$, alors
\[
...
$f(x)=N(x)/D(x)\le t+v/D(x)\le t+v/D_{\min}$ puisque $v\ge0$. \end{proof}
