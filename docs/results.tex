...
\emph{probe-count} saving is capped at one over that figure, and shrinks as it
grows: sweeping $p$ at $n=7$ gives probes-per-vector $2.3$, $3.0$, $4.0$,
$4.9$, $6.4$, $7.5$ and probes kept $0.68$, $0.75$, $0.82$, $0.87$ --- $32\%$
removed at $p=3$ down to $13\%$ at $p=6$, over 72 instances with 68 faster.

\begin{quote}
That bound is exact and it is about \emph{counts}.  \textbf{It does not bound
the time}, because a seed removes a probe on a non-empty box --- which carries
an efficiency test behind it --- while the probes left behind are mostly on
empty boxes and are refuted at once.  At $p=6$ the time ratio exceeds
$1/(\text{probes kept})$.  An earlier draft here called it ``a hard ceiling of
roughly $1.4\times$''; that is withdrawn.
\end{quote}

\noindent
What does grow with $|F|$ is the walk's cost, one efficiency test per archive
member --- so a large front buys no more and costs more.
