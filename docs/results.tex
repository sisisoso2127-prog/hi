...
\begin{table}[h]
\centering
\caption{Decision space against criterion space, $n = 6$, four instances per
row.  All twenty agreed and were proved optimal on both sides.}
\begin{tabular}{crrr}
\toprule
...
\end{center}
\noindent
\emph{The bars are clipped at 30\,s so the small $p$ stay visible; the $p=5$
decision-space bar runs off the top at $112.77$\,s.}
...
against the prediction made before running it: branching multiplies a
\emph{count} of cheap, fixed-size models, while the binaries multiply the
\emph{cost of one model}, and they accumulate.
...
instance.  The table reports what was measured, not an expected value.
...
\begin{table}[h]
\centering
\caption{The thirteen things tried, with what each was worth.}
\begin{tabular}{lll}
\toprule
...
\noindent
Six are losses and one is a tie.  They are reported at the same length as the
...
third-party dependency, and 74 tests run on every commit.  Several check
