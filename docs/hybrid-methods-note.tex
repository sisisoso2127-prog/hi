...
\begin{tabular}{lrrr}
\toprule
 & decision space & criterion space & ratio \\
\midrule
$n = 4$, 29 non-dominated vectors & $2.11$\,s & $0.32$\,s & $6.68\times$ \\
\bottomrule
\end{tabular}
\end{table}

\noindent
\emph{One size only.}  Enumerating the whole front in decision space becomes
slow quickly, and larger $n$ was not measured here; the row above is what was
run, not a trend.  The direction is the one Section~\ref{sec:results} measures
at length for the optimisation search, but it is not asserted for enumeration
beyond this row.
