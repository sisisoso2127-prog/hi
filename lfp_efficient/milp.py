...
def solve_milp(model: Model, objective, max_nodes: int = 200_000) -> MilpResult:
    ...
    *objective* is either a list of coefficients (linear) or a
    ...
    the first integer-constrained variable with a fractional value, using the
    usual dichotomy ``x_j <= floor(v)`` / ``x_j >= ceil(v)``.
    """
    ...
    while stack:
        ...
        # bound: the relaxation cannot beat the incumbent -> prune
        if best.feasible and relax.objective <= best.objective:
            continue

        x = relax.x[:node.n]
        frac = next((j for j in range(node.n)
                     if node.integrality[j] and not is_integral(x[j])), None)
...
def solve_fractional_milp(model: Model, phi, max_nodes: int = 200_000) -> MilpResult:
    ...
    for sign in (+1, -1):
        ...
        if sign > 0:
            ...
            objective = phi.__class__([-u for u in phi.U], [-v for v in phi.V],
                                      -phi.alpha, -phi.beta)
        res = solve_milp(branch, objective, max_nodes=max_nodes)
        nodes += res.nodes
